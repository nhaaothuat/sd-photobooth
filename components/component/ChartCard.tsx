...
const RevenueChart = () => {
     ...
     useEffect(() => {
          const fetchData = async () => {
               try {
                    const response = await AxiosAPI.get<RevenueResponse>("/api/Dashboard/statictis-revenue-by-platform-type", {
                         params: { groupingType },
                    });
                    if (response.data) {
          ...
          <Card className="p-6 shadow-md rounded-2xl bg-white">
               <CardHeader className="mb-4">
                    <div className="flex justify-between items-center">
                         <h2 className="text-xl font-semibold">Doanh thu theo nền tảng</h2>
                         <Select
