     ...
     return (
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
               <DialogTrigger asChild>
                    <Button variant="outline"><CirclePlus className="mr-2" /> Add Style</Button>
               </DialogTrigger>
               <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                         <DialogTitle>Add Photo Style</DialogTitle>
                         <DialogDescription>Fill the form to add a new photo style.</DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                         <div>
                              ...
                              {errors.imageFile && <p className="text-sm text-red-500">{errors.imageFile.message}</p>}
                         </div>
                         <div>
                              <Label>Name</Label>
                              <Input {...register("name")} />
                         ...
                         <div className="grid grid-cols-2 gap-4">
                              <div><Label>Images/Gen</Label><Input type="number" {...register("numImagesPerGen")} /></div>
                              <div><Label>Background</Label><Input type="color" {...register("backgroundColor")} /></div>
                              <div><Label>Height</Label><Input type="number" {...register("height")} /></div>
                              <div><Label>Width</Label><Input type="number" {...register("width")} /></div>
                              <div><Label>Mode</Label><Input type="number" {...register("mode")} /></div>
                              <div><Label>Inference Steps</Label><Input type="number" {...register("numInferenceSteps")} /></div>
                              <div><Label>Guidance Scale</Label><Input type="number" step="0.1" {...register("guidanceScale")} /></div>
                              <div><Label>Strength</Label><Input type="number" step="0.1" {...register("strength")} /></div>
                         </div>
                         <div className="flex items-center space-x-4">
                              <Switch checked={watch("faceImage")} onCheckedChange={(checked) => setValue("faceImage", checked)} />
                              <Label>Face Image</Label>
                         </div>
                         <div className="flex items-center space-x-4">
                              <Switch checked={watch("backgroundRemover")} onCheckedChange={(checked) => setValue("backgroundRemover", checked)} />
