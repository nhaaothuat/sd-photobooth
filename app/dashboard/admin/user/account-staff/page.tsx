...
import { Table, Text, ScrollArea, Group, Badge, Center, UnstyledButton } from "@mantine/core";
...
const AccountPageStaff = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const filteredUsers = userData.filter(user => user.role === "Staff");
            ...
            {users.length > 0 ? (
              users.map((user) => (
                <Table.Tr key={user.id}>
                  ...
                  <Table.Td>
                    <Badge color="yellow">{user.role}</Badge>
                  </Table.Td>
                </Table.Tr>
              ))
            ) : (
...
export default AccountPageStaff;
