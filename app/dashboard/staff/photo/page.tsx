...
const ByPhotoHistory = () => {
  ...
  const handleFetchStickers = async () => {
    ...
    try {
      const response = await AxiosAPI.get<PhotoStyle[]>(`/api/PhotoHistory/manage/photo/${id}`)
        ...
        <div>
          ...
          <SimpleGrid cols={6} spacing="lg" verticalSpacing="sm">
            {stickers.map((sticker) => (
              <div key={`${sticker.id}-${sticker.name}`} className="flex flex-col items-center">
                {sticker.imageUrl ? (
                  <Indicator inline label={sticker.name || 'No name'} color="blue" size={16}>
                    <Image
