          ...
          <div className="p-4 max-w-xl mx-auto bg-white rounded shadow">
               ...
               {!error && customer && (
                    <div className="border-t pt-4 space-y-2">
                         ...
                         <p><strong>Họ tên:</strong> {customer.fullName || 'Chưa có'}</p>
                         <p><strong>Tên người dùng:</strong> {customer.userName}</p>
                         <p><strong>Email:</strong> {customer.email}</p>
                         <p><strong>Số điện thoại:</strong> {customer.phoneNumber || 'Chưa có'}</p>
                         <p><strong>Giới tính:</strong> {customer.gender === 0 ? 'Nam' : customer.gender === 1 ? 'Nữ' : 'Khác'}</p>
                         <p><strong>Ngày sinh:</strong> {customer.birthDate || 'Chưa có'}</p>
                         <p><strong>Vai trò:</strong> {customer.role}</p>
                         <p><strong>Bị cấm:</strong> {customer.isBanned ? 'Có' : 'Không'}</p>
                    </div>
               )}
          </div>
     );
